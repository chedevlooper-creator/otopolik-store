  ...
  return (
    <footer className="border-t border-white/8 bg-[#07080a] text-white">
      <div className="mx-auto max-w-7xl px-4 pt-5">
        <div className="relative overflow-hidden rounded-[1.5rem] border border-brand-red/25 bg-gradient-to-br from-brand-red via-[#c5112d] to-[#790b1c] px-6 py-8 shadow-[0_30px_90px_rgba(227,25,55,.16)] sm:px-9 sm:py-10">
      ...
      <div className="mx-auto max-w-7xl px-4 py-14 sm:py-18">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-[1.35fr_.75fr_.75fr_1fr] lg:gap-12">
          ...
          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-[0.16em] text-sand">Keşfet</h3>
            <ul className="mt-5 space-y-3.5 text-sm">
              {PAGE_LINKS.map((item) => (
                <li key={item.href}><Link href={item.href} className="text-white/62 transition-colors hover:text-white">{item.label}</Link></li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-[0.16em] text-sand">Destek</h3>
            <ul className="mt-5 space-y-3.5 text-sm">
              {INFO_LINKS.map((item) => (
                <li key={item.href}><Link href={item.href} className="text-white/62 transition-colors hover:text-white">{item.label}</Link></li>
              ))}
