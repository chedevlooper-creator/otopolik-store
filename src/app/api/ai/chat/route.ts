...
import { configuratorTools } from "@/lib/ai/configurator-tools";
...
export async function POST(request: NextRequest) {
  ...
  const model = getLanguageModel("configurator-chat");
  if (!model) {
    return NextResponse.json({ error: "ai_unavailable" }, { status: 503 });
  }
  ...
  try {
    ...
    const result = streamText({
      ...
      tools: configuratorTools,
      maxOutputTokens: getAiMaxTokens("configurator-chat"),
    ...
    console.error(
      "Konfigüratör sohbeti başlatılamadı:",
