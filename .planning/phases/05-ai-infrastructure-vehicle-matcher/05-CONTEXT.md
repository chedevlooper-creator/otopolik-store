...
- **Placement:** Customer-facing LLM calls live in Next.js route handlers under `src/app/api/ai/*` (server-only key, SSE-capable). Do NOT put the provider key in the browser or call the LLM client-side.
- **Vehicle match strategy:** deterministic fuzzy/token match via existing `src/lib/vehicle-search.ts` FIRST; the LLM is only a fallback to parse messy free text into brand/model/year/trim tokens and to disambiguate 0-/multi-match results. Never replace the existing matcher.
- **Ground truth:** `vehicle-data.ts` for brand/model/bodyType; `mat-pricing.ts` `getVehiclePrice()` / `calculateMatPrice` for price tier. The model NEVER free-generates a price (AIINF-03).
- **Provider/SDK:** Vercel AI SDK (`ai` + provider adapter) as the streaming/tool-calling abstraction; use the cheap fast model for the closed-set matcher classification. Verify exact npm version pairing at install time.
- **Graceful fallback (AIINF-02):** mirror the Convex-first + static-fallback pattern — if the provider key is unset/placeholder, the AI match entry point hides/disables and the user falls back to the existing manual vehicle dropdowns. Nothing breaks.
- **Cost controls (AIINF-04):** per-session/IP rate limiting, per-feature `max_tokens` caps, and a feature-flag kill switch shipped WITH the first public endpoint, not retrofitted.
- **Tool-surface isolation (AIINF-05):** the vehicle-match tool operates only over the static in-memory `vehicle-data.ts` array — it shares no tool surface with any `requireAdminKey()`-gated mutation.
- **Eval harness (AIINF-06):** a golden dataset (20–30 Turkish free-text vehicle cases) with price-equality assertions and Turkish-quality checks, runnable before any prompt/model change. Decide harness home (local script vs CI) during planning.
